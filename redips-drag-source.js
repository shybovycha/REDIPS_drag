...
REDIPS.drag = (function () {
    ...
    objMargin = null, // space from clicked point to the object bounds (top, right, bottom, left)
        ...
        edge = {
            page: {
                x: 0,
                y: 0
            }, // autoscroll bound values for page and div as scrollable container
            div: {
                x: 0,
                y: 0
        ...
        topLevelContainer = null, // top-level container reference
        divBox = null, // div drag box: top, right, bottom and left margin (decrease number calls of setTableRowColumn)
        pointer = {
            x: 0,
            y: 0
        }, // mouse pointer position (this properties are set in handlerOnMouseMove() - needed for autoscroll)
        threshold = {
        ...
        trash = {
            className: 'trash', // (object) contains trash class name and confirmation questions for delete DIV or ROW
            question: null,
            questionRow: null
        },
     ...
     * @param {String} [tlc] Top-level container Id.
     * @example
    ...
    init = function (dc, tlc) {
        ...
        // set the reference to the top-level container
        topLevelContainer = document.getElementById(tlc);
    ...
    handlerOnMouseUp = function (e) {
        ...
        // if DIV element was clicked and left button was released, but element is placed inside unmovable table cell
            ...
            else if (cloned && REDIPS.drag.clone.drop === false && (X < target_table.redips.offset[3] || X > target_table.redips.offset[1] || Y < target_table.redips.offset[0] || Y > target_table.redips.offset[2])) {
                obj.parentNode.removeChild(obj);
                // decrease clonedId counter
                clonedId[objOld.id] -= 1;
                REDIPS.drag.event.notCloned();
    ...
    handlerOnMouseMove = function (e) {
        ...
        if (X < divBox[1] && X > divBox[3] && Y < divBox[2] && Y > divBox[0] && edge.flag.x === 0 && edge.flag.y === 0 && (currentCell.containTable || (X < currentCell[3] || X > currentCell[1] || Y < currentCell[0] || Y > currentCell[2]))) {
    ...
    setTableRowColumn = function () {
        ...
        for (table = 0; table < tables.length; table++) {
            ...
            if (tos[3] < X && X < tos[1] && tos[0] < Y && Y < tos[2]) {
                ...
                while
                (tables[table].redips.rowspan && cell === -1 && row-- > 0);
    ...
    getScrollPosition = function () {
        ...
        if (typeof (window.pageYOffset) === 'number') {
            scrollOffsets.push([window.pageXOffset, window.pageYOffset, 'first']);
        }

        // DOM compliant
        if (document.body && (document.body.scrollLeft || document.body.scrollTop)) {
            scrollOffsets.push([document.body.scrollLeft, document.body.scrollTop, 'second']);
        }

        // IE6 standards compliant mode
        if (document.documentElement && (document.documentElement.scrollLeft || document.documentElement.scrollTop)) {
            scrollOffsets.push([document.documentElement.scrollLeft, document.documentElement.scrollTop, 'third']);
        }

        // Other scrolling
        if (topLevelContainer && (topLevelContainer.scrollLeft || topLevelContainer.scrollTop)) {
            scrollOffsets.push([topLevelContainer.scrollLeft, topLevelContainer.scrollTop, 'fourth']);
        }

        // Other scrolling
        if (dragContainer && (dragContainer.scrollLeft || dragContainer.scrollTop)) {
            scrollOffsets.push([dragContainer.scrollLeft, dragContainer.scrollTop, 'fifth']);
        }

        // needed for IE6 (when vertical scroll bar was on the top)
        scrollOffsets.push([document.documentElement.scrollLeft, document.documentElement.scrollTop, 'sixth']);

        var result = scrollOffsets.reduce(function (previousValue, currentValue, index, array) {
    ...
    moveObject = function (ip) {
        var p = {
            'direction': 1
        }, // param object (with default direction)
